...
const PoemViewer = ({ poem, metadata }) => {
  return (
    <div className="max-w-3xl mx-auto bg-gradient-to-b from-amber-50 to-amber-100 rounded-lg shadow-xl p-8 border border-amber-300">
      ...
      {/* Main content with scroll area for longer poems */}
      <div className="relative bg-amber-50/80 rounded-lg p-6 shadow-inner border border-amber-200 min-h-[300px] max-h-[500px] overflow-y-auto">
        ...
        <div className="relative px-8 py-4">
          <pre className="whitespace-pre-wrap font-serif text-xl leading-relaxed text-amber-900 text-center italic">
            {poem}
          </pre>
        </div>
      </div>
      ...
      <div className="mt-6 pt-4 border-t border-amber-300 flex justify-between items-center">
        <div className="flex items-center space-x-2 text-sm text-amber-800">
          ...
          <span className="px-3 py-1 bg-amber-200 rounded-full">{metadata.language.toUpperCase()}</span>
        </div>
        <div className="flex items-center space-x-2 text-sm text-amber-800">
          <span className="px-3 py-1 bg-amber-200 rounded-full">{metadata.sentence_count} lines</span>
          {metadata.theme && <span className="px-3 py-1 bg-amber-200 rounded-full">{metadata.theme}</span>}
        </div>
      </div>
      
      {/* Optional: Add decorative footer */}
...
const PoemGenerator = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setPoem({ error: 'Failed to generate poem. Please try again.' });
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="w-full h-screen max-h-screen p-4 bg-gray-100">
      <div className="container mx-auto h-full max-w-6xl border border-gray-300 rounded-lg shadow-lg flex flex-col bg-white">
        ...
        <div className="flex items-center p-6 border-b border-gray-200 bg-gradient-to-r from-red-600 to-red-800">
          <h1 className="text-lg font-bold text-white">AI Poem Generator</h1>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <input
                type="text"
                name="theme"
                placeholder="Enter poem theme (optional)"
        ...
        <div className="flex-1 p-6 overflow-y-auto">
          ...
          {poem && !poem.error && !pdfUrl && (
            <PoemViewer 
              poem={poem.poem} 
              metadata={{
                created_at: poem.created_at,
                sentence_count: poem.sentence_count,
